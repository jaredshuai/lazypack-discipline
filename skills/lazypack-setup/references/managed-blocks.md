...
| `src` | 派生固定层版本 | `DECISIONS.md@0.3.0` | 格式为 `DECISIONS.md@<SemVer>`（历史 0.2.0 产物在重跑时按决策树触发 [UPGRADE]） |
...
### 5.5 Section 3 非受管协作区追加、细粒度定位与安全写前契约 (P6)
- **物理边界与块外解耦保护**：`docs/ARTIFACTS.md` 的 Section 3 位于 `<!-- lazypack:end block=artifacts-register -->` 之后，属于非受管协作区。Section 3 的内容与追加不影响 Section 2 受管块的 `header.fp` 与 `header.input`，彻底杜绝伪 DRIFT。
- **旧项目缺节平滑追加**：既有项目的 `docs/ARTIFACTS.md` 若无 Section 3，在确认写盘时将 Section 3 模板框架追加至文件末尾，块内字节逐字节不变，不影响 `fp` 计算。
- **去重键与细粒度定位契约**：
  1. **唯一去重键**：
     - 仓内产物支持 POSIX 规范相对路径（`path`）或章节锚点细粒度定位（`path#anchor`，如 `README.md#快速开始`）；
     - 同一文件不同章节锚点（如 `README.md#快速开始` 与 `README.md#架构总览`）合法共存；
     - 仓外引用支持规范化安全别名（如 `vault:raw-qa`）或真实 HTTP/HTTPS URL；
     - 去重匹配使用规范化身份（去重键完全比对），不使用模糊匹配。
  2. **四类登记对象的存在性与核验契约**：
     - **本地文件与有效章节锚点 (Local Files & Sections)**: 物理文件在磁盘真实存在且非空，有效章节锚点必须在文件内存在对应标题；
     ...
     - **留存协作种子与私有存储绑定 (Retention Seeds & Vault Bindings)**: 严格维持现有非空与 `session-authorized` 契约（ideas 种子必须非空；raw_qa 必须经当前会话显式绑定且不向公开产物暴露绝对私有路径，未绑定严格保持 `unbound` 零写入零预登记，**绝不为普通文档放宽私有存储安全防线**）。
  3. **不重复添加（幂等追加）**：若 Section 3 表格已存在该去重键，不重复写入行，保持原样；保留已有普通文件键，升级定位规则时严禁重复追加旧条目。
  4. **旧表头、手工注释与人工行绝对保护**：
     - Section 3 原有的非标准表头、手工注释、前置说明及人工登记行，setup 重跑时绝对不删除、不覆写、不重排，原有字节完全保护；
     - 若存量旧表头无法安全自动映射，在 Section 3 安全边界追加独立兼容表格，严禁重排旧表；
     - 存量同键但归属/状态建议调整时：首期 setup 保持原手工行不覆写并向用户报告待裁，绝不单方面覆盖。
- **写前重读、内存合并与统一单次写盘契约 (Pre-write Re-read, In-Memory Merge & Single Atomic Write)**：
  1. **防范自身写入误判并发**: `docs/ARTIFACTS.md` 包含受管区（Section 2）与协作区（Section 3）。若分次写盘，受管块的落盘会导致文件实时哈希改变，使后续 Section 3 的写前重读误判自身写入为“外部并发修改”而触发伪 `PAUSE`。因此，必须采用**内存统一合并、写前统一校验、单次原子写盘**机制；
  2. **前置门禁裁决**: 在写盘前首先裁决前置保护与零写入门禁（若文件处于 PAUSE/BROKEN/未托管，保持整文件保护；若同主题冲突或留存策略处于 pending 或 declined，对应项的零写入门禁必须在写入前判断拦截，严禁写入后再宣布零写入）；
  3. **统一写前重读比对**: 在物理写盘前，重新读取磁盘目标文件（若为全新文件则核验磁盘尚不存在）；比对实时磁盘哈希是否等于第 4 步记录的统一写前基线哈希 $H_{\text{baseline}}$。若哈希不符（发生外部并发写入），**立即触发 PAUSE 暂停写盘并告警**；绝对禁止覆写外部改动，亦不执行回滚；绝不将外部并发变更吸收为己方基线；
  4. **内存合并与单次写盘**: 重读比对通过后，在内存中完成受管块（重新采样 Layer 0 依赖状态生成最终 `header.input` 与 `fp`）与 Section 3 候选追加项（按规范 5 列推荐表头、去重键与换行规范）的完整合并，随后执行**单次物理落盘**，写后立即读回核验。
- **原生换行与末尾追加规范**：
  1. 原样保留文件开头的 UTF-8 BOM 与原生换行格式（CRLF 或 LF）；
  2. 若目标文件末尾缺少换行符，向末尾追加新行时需先插入一个与文件主流换行符一致的分隔换行符；该新增分隔符属于追加操作的技术实现，原有内容前缀 Buffer 保持逐字节相等。
- **受管块 NO-OP 与 Section 3 追加正交性**：
  1. 受管块判定为 `[NO-OP]`（正文与 input 未变），绝不阻断 Section 3 经用户整体确认后的新批准条目追加；
  2. 无任何新增项且无变更重跑时，整文件保持 0 磁盘写入与 0 重复确认；
  3. 仅影响 Section 3 登记的参考规则（如 `references/document-routing.md`）变动时，不计入受管块 `header.input`，避免受管区假 DRIFT/假 UPGRADE。
