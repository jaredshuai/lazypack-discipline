...
- **Y**：链接目标在磁盘上存在且非空
- **判定**：去掉围栏代码与行内反引号后再抽链。跳过带协议的 URL（含 `http(s):`、`file:`、`mailto:`、`ftp:`）与纯锚点。相对路径去掉 fragment 后相对本文件解析。普通文件 `size > 0`；目录至少有一个条目。反引号中的叙事路径不是链接，不检查。

### P5 入口文件相对链接可解析

- **X**：`AGENTS.md` 与 `README.md` 内的标准 Markdown 内联相对链接
- **Y**：链接目标存在
- **判定**：抽链规则与 P4 相同（同样跳过协议 URL 与反引号）。本条只要求目标存在，不额外要求非空。

### P8 交接文档 CLI 与实现集合一致

- **X**：`docs/agents/handoff-verification.md` §2 声明的子命令与 flag
- **Y**：`scripts/handoff_manifest.js` 的 CLI 解析分支（`command ===` + `getOption` / `hasFlag`）
- **判定**：子命令集合相等；每个子命令的 flag 集合相等。多余或缺失任一侧即 FAIL。
